...
#Private Target Encoder
def encoder(x, name='priviate_target_encoder', reuse=False):
    with tf.variable_scope(name) as scope:
        if reuse:
            scope.reuse_variables()
        x = (tf.cast(x, tf.float32) - pixel_mean) / 255.
        print (x.shape)
        net = slim.conv2d(x, 32, [5, 5], scope='conv1encoder')
        print (net.shape)
        net = slim.max_pool2d(net, [2, 2], scope='pool1encoder')
        print (net.shape)
        net = slim.conv2d(net, 48, [5, 5], scope='conv2encoder')
        print (net.shape)
        net = slim.max_pool2d(net, [2, 2], scope='pool2encoder')
        print (net.shape)
        feat = slim.flatten(net, scope='flatencoder')
    return feat
...
def shared_decoder(feat,height,width,channels,reuse=False, name='shared_decoder'):
    with tf.variable_scope(name) as scope:
        if reuse:
            scope.reuse_variables()
    net = slim.fully_connected(feat, 600, scope='fc1')
